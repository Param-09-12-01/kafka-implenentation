...
public class KafkaFailedServiceLogSender {
    ...
    public void publishFailedServiceLogEvent(FailedSvcLogEvent event) {
        LOG.info("Publishing failed service log event to Kafka. topic: {}, svcName: {}, failedTime: {}",
                InventoryTopicConstant.FAILED_SERVICE_LOG_TOPIC, event.getSvcName(), event.getFailedTime());
        failedServiceLogKafkaTemplate.send(InventoryTopicConstant.FAILED_SERVICE_LOG_TOPIC, event).whenComplete((result, ex) -> {
            if (ex != null) {
                LOG.error("Failed to publish failed service log event. topic: {}, svcName: {}, failedTime: {}",
                        InventoryTopicConstant.FAILED_SERVICE_LOG_TOPIC, event.getSvcName(), event.getFailedTime(), ex);
                return;
            }
