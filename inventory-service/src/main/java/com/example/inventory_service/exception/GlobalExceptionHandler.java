...
import com.example.inventory_service.kafka.KafkaFailedServiceLogSender;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleIllegalArgumentException(IllegalArgumentException ex, HttpServletRequest request) {
        LOG.warn("Bad inventory-service request. path: {}, message: {}", request.getRequestURI(), ex.getMessage());
    ...
    public ResponseEntity<ApiError> handleNoSuchElementException(NoSuchElementException ex, HttpServletRequest request) {
        LOG.warn("Inventory resource not found. path: {}, message: {}", request.getRequestURI(), ex.getMessage());
        publishFailedServiceLog(ex, request);
    ...
    public ResponseEntity<ApiError> handleDataIntegrityViolationException(DataIntegrityViolationException ex, HttpServletRequest request) {
        LOG.warn("Inventory database constraint failed. path: {}", request.getRequestURI(), ex);
        publishFailedServiceLog(ex, request);
    ...
    public ResponseEntity<ApiError> handleException(Exception ex, HttpServletRequest request) {
        LOG.error("Unhandled inventory-service exception. path: {}", request.getRequestURI(), ex);
        publishFailedServiceLog(ex, request);
